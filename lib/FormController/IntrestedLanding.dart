      ...
      body: Column(
        ...
        children: [
                      ...
                      InkWell(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            borderRadius: BorderRadius.circular(10),
                            border: Border.all(
                              color: _yesSelected ? AppColors.primaryColor : Colors.black,
                              width: 1,
                            ),
                          ),
                          child: Padding(
                            ...
                            child: Row(
                              children: [
                                Icon(Icons.circle_outlined, color: _yesSelected ? AppColors.primaryColor : Colors.black),
                                SizedBox(width: 5),
                                Text(
                                  'YES',
                                  style: TextStyle(
                      ...
                      InkWell(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            borderRadius: BorderRadius.circular(10),
                            border: Border.all(
                              color: _noSelected ? AppColors.primaryColor : Colors.black,
                              width: 1,
                            ),
                          ),
                          child: Padding(
                            ...
                            child: Row(
                              children: [
                                Icon(Icons.circle_outlined, color: _noSelected ? AppColors.primaryColor : Colors.black),
                                SizedBox(width: 5),
                                Text(
                                  'NO',
                                  style: TextStyle(
          ...
          if (searchResults != null && searchResults!.isNotEmpty) ...[
            SizedBox(height: 10), // Padding between content and list
            Expanded(
               child: Padding(
              padding: const EdgeInsets.symmetric(horizontal: 12.0),
              child: Container(
                padding:  EdgeInsets.symmetric(vertical: 8.0), // Padding for the list
                decoration: BoxDecoration(
                  color: Colors.white,
                  border: Border.all(color: Colors.grey), // Border around the list
              ...
              onTap: () {
                setState(() {
                  selectedRowIndex = index; // Update the selected row index
                });
                // Handle tapping on the list item
                String customerID = searchResults![index]['CustomerID'];
                CustomerIDStorage.setCustomerID(customerID);
                _showNextButton = true;
              },
              child: Container(
                color: isSelected ? Colors.blue.withOpacity(0.2) : Colors.transparent, // Change the background color for the selected row
                padding: EdgeInsets.symmetric(horizontal: 16.0), // Add padding to the tile
                child: ListTile(
                  title: Text('${searchResults![index]['FirstNameP']} ${searchResults![index]['LastNameP']}'),
                  subtitle: Text('Customer ID: ${searchResults![index]['CustomerID']}'),
                ),
              ),
            );
                  },
...
void handleCustomerSelection(String selectedOption) {
 ...
 List<Map<String, dynamic>> results = await searchByFirstName(selectedOption);
  ...
  Future<List<Map<String, dynamic>>> searchByFirstName(String searchString) async {
...
for (Map<String, dynamic> result in results) {
    ...
    print('First Name: $firstName, Last Name: $lastName, Customer ID: $customerId');
  }
