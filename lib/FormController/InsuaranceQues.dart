                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              BulletSelectionWidget(
                                  question:
                                      'Was there a change in weight of more than 5 kg in the last one year?',
                                  options: const ['NO', 'YES'],
                                  selectionController: _weightChange),
                              const SizedBox(height: 8),
                              buildFormField(
                                  'If yes reason for same', _weightReason),
                              const SizedBox(height: 8),
                              BulletSelectionWidget(
                                  question:
                                      'COVID-19 (excluding mandatory government orders to remain at home) or had a persistent cough, fever, raised temperature or been in contact with an individual suspected or confirmed to have COVID-19?',
                                  options: const ['NO', 'YES'],
                                  selectionController: _covidSymtoms),
                              const SizedBox(height: 8),
                              BulletSelectionWidget(
                                  question:
                                      'Did you suffer from any complications of lung (respiratory), kidney, liver, or heart problems related to the COVID-19 infection or Long COVID?',
                                  options: const ['NO', 'YES'],
                                  selectionController: _covidSuffer),
                              const SizedBox(height: 8),
                              buildFormField(
                                  'If yes reason for same', _covidReason),
                              const SizedBox(height: 8),
                              buildDropdownField('Relation Ship with Applicant',
                                  ['Mother', 'Child'], _relationship),
                              const SizedBox(height: 8),
                              buildFormField('Nominee First Name', _firstName),
                              const SizedBox(height: 8),
                              buildFormField('Nominee Surname', _surname),
                              const SizedBox(height: 8),
                              buildDropdownField('Nominee Gender',
                                  ['Male', 'Female'], _gender),
                              const SizedBox(height: 8),
                              buildDateOfBirthField(
                                  context, 'Nominee DOB', _dob),
                              const SizedBox(height: 8),
  ...
  Future<void> getInsuranceDetailsForClientFromSQLite(String clientId) async {
    ...
    if (insuranceData.isNotEmpty) {
      setState(() {
        ...
        _weightChange.text = insuranceData[0]['WeightChange'] ?? '';
        _weightReason.text = insuranceData[0]['WeightChangeReason'] ?? '';
        _covidSymtoms.text = insuranceData[0]['CovidSymptoms'] ?? '';
        _covidSuffer.text = insuranceData[0]['CovidComplications'] ?? '';
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: const Text('Insuarance details saved successfully!'),
          actions: <Widget>[
            TextButton(
              onPressed: () {
