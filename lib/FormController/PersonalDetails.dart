...
class _PersonalDetailsState extends State<PersonalDetails> {
  ...
  // late Database _db;
   ...
   final TextEditingController _applicantNameControllerPAN = TextEditingController();
   final TextEditingController _applicantNameControllerAdhar = TextEditingController();
  final TextEditingController _mobileNoController = TextEditingController();
  final TextEditingController _mobileNoSecondaryController = TextEditingController();
  final TextEditingController _emailIdController = TextEditingController();
  final TextEditingController _ageController = TextEditingController();
  final TextEditingController _dobController = TextEditingController();

  final TextEditingController _customerEntityTypeController = TextEditingController();
   final TextEditingController _applicantTypeController = TextEditingController();
                  ...
                  child: SingleChildScrollView(
                    child: Column(
                      ...
                      children: [
                        ...
                        buildFormField('Customer ID', _customerIdController),
                        /*CustomFormField(
                          title: 'Customer ID', 
                        ...
                        const SizedBox(height: 10),
                        UploadPhotoField(textEditingController: _imagePathController, selectedImagePath: imagePath,),
                        const SizedBox(height: 10),
                        buildDropdownField('Customer Entity Type', ['Individual'], _customerEntityTypeController, onChanged: handleDropdownChange, dropdownKey: _dropdownEntityType),
                        const SizedBox(height: 10),
                        buildDropdownField('Applicant Type', ['Individual'], _applicantTypeController, dropdownKey: _dropdownApplicationType),
                        const SizedBox(height: 10),
                        buildDropdownField('Salutation', ['Mr', 'Mrs'], _salutationController, dropdownKey: _dropdownSolutation),
                        const SizedBox(height: 10),
                        _buildNameFields('First Name', 'Middle Name', 'Last Name'),
                        const SizedBox(height: 10),
                        buildFormField("Applicant name as per PAN", _applicantNameControllerPAN),
                        const SizedBox(height: 10),
                        buildFormField("Applicant name as per Adhar", _applicantNameControllerAdhar),
                        const SizedBox(height: 10),
                        buildDropdownField("Religion", ["Hinduism", "Muslim", "Bhudhism" ], _relionController, dropdownKey: _dropdownReligion),
                        const SizedBox(height: 10),
                        buildDropdownField("Caste", ["General", "OBC" ], _castController, dropdownKey: _dropdownCastType),
                        const SizedBox(height: 10),
                        buildDropdownField("Occupation", ["Sales", "Backend" ], _occupationController, dropdownKey: _dropdownOccupation),
                        const SizedBox(height: 10),
                        buildDropdownField("Education Qualification", ["Engineer", "Doctor", "Other" ], _educationController, dropdownKey: _dropdownQualification),
                        const SizedBox(height: 10),
                        DropDownWithItem(title: 'Mobile Number', controller: _mobileNoController),
                        const SizedBox(height: 10),
                        DropDownWithItem(title: 'Phone no. - Secondary', controller: _mobileNoSecondaryController),
                        const SizedBox(height: 10),
                        buildFormField('Email Id', _emailIdController),
                        const SizedBox(height: 10),
                        buildDateOfBirthField(context, 'Date Of Birth', _dobController),
                        const SizedBox(height: 10),
                        buildFormField('Age', _ageController),
                        const SizedBox(height: 10),
                        buildDropdownField('Marital Status', ['Married', 'Unmarried'], _maritalStatusController, dropdownKey: _dropdownMaritalStatus)                        
...
Widget _buildNameFields(String firstNameTitle, String middleNameTitle, String lastNameTitle) {
  ...
  return Column(
    ...
    children: [
      ...
      buildFormField(middleNameTitle,_middleNameController ,isMandatory: true),
      const SizedBox(height: 8.0),
...
await db.insert(
    ...
    conflictAlgorithm: ConflictAlgorithm.replace, // You can define the conflict algorithm as per your needs
  );
...
Future<void> updateCustomerIDForCKYCNumber(String ckycNumber, String newCustomerID) async {
  Database db = await DatabaseHelper.instance.database;
...
void getDataForUserFromSQLite(String userId) async {
  ...
  if (userData.isNotEmpty) {
    setState(() {
      ...
      _applicantNameControllerPAN.text = userData[0]['ApplicantNamePAN'] ?? '';
      _applicantNameControllerAdhar.text = userData[0]['ApplicantNameAdhar'] ?? '';
      _mobileNoController.text = userData[0]['MobileNumberP'] ?? '';
      _mobileNoSecondaryController.text = userData[0]['PhoneNoSecondary'] ?? '';
      _emailIdController.text = userData[0]['EmailId'] ?? '';
      _ageController.text = userData[0]['Age'] ?? '';
      _dobController.text = userData[0]['DOBP'] ?? '';
      _customerEntityTypeController.text = userData[0]['CustomerEntityType'] ?? '';
